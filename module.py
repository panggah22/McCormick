...
def mdt_ii(m,x,y,datapair,p=0,P=0):
    ...
    m.addConstrs((y[i,t] == gp.quicksum(hat_x[i,t,k,l] for k in set_k) for l in set_l for i,t in pairs))

    m.addConstrs((hat_x[i,t,k,l] >= y[i,t].LB * z[i,t,k,l] for i,t,k,l in set_kl))
    m.addConstrs((hat_x[i,t,k,l] <= y[i,t].UB * z[i,t,k,l] for i,t,k,l in set_kl))

    m.addConstrs((z.sum(i,t,'*',l) == 1 for i,t,k,l in set_z))

    m.addConstrs((delta_w[i,t] >= y[i,t].LB * delta_x1[i,t] for i,t in pairs))
    m.addConstrs((delta_w[i,t] <= y[i,t].UB * delta_x1[i,t] for i,t in pairs))

    m.addConstrs((delta_w[i,t] <= (y[i,t] - y[i,t].LB) * 10**p + y[i,t].LB * delta_x1[i,t] for i,t in pairs))
    m.addConstrs((delta_w[i,t] >= (y[i,t] - y[i,t].UB) * 10**p + y[i,t].UB * delta_x1[i,t] for i,t in pairs))
...
def var_df(x_ax,y_ax,y,mult=1):
    a = np.array(var_values(y,mult))
    return a.reshape(x_ax,len(y_ax))

def mdt_iij_bin(m,x,y,datapair,p=0,P=0):
    pairs = datapair

    idx_x = [idx for idx,v in x.items()]

    set_l = range(p,P+1)
    set_kl = tuplelist([(*g,k,l) for l in set_l for g in pairs])
    set_z = tuplelist([(i,t,k,l) for l in set_l for i,t in idx_x])
